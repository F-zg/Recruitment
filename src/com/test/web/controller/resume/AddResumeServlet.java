...
public class AddResumeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(resume1!=null) {
			PrintWriter out=response.getWriter();
			  out.print("<script type='text/javascript' charset='UTF-8'>");
				out.print("alert('a user can not have two resume');");
				out.print("window.location='logenterpriseUser.jsp';");
				out.print("</script>");
		}
		String rcompany = request.getParameter("rcompany");
		String position = request.getParameter("position");		// 应该使用 md5|sha1
		...
		Resume resume = new Resume( rcompany,  position, workStartTime,  workEndTime,  salary,duty,  schoolName,  education, professional,  entranceTime,  graduateTime,0, userId);
		
		boolean result = resumeService.register(resume);
		if (result) {
			// 转发跳转到index.jsp
